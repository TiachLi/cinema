...
/*    public static List<JSONObject>  bulid(Permission topNode,List<Permission> treeNodes){
        if (topNode.getChildren().size()>0){
            ArrayList<JSONObject> children = new ArrayList<>();
            for (Permission two : treeNodes) {
                ...
                List<JSONObject> bulid = bulid(two, treeNodes);
                twoMeun.put("children",bulid);
